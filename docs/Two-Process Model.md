...
- `command_sync` and `input_sync`: launcher **binds**, renderer **connects**. On Windows the launcher stamps an Untrusted mandatory label (`S:(ML;;NW;;;S-1-16-0)`) + permissive DACL on the bound socket file via `SetNamedSecurityInfo`, otherwise Windows Mandatory Integrity Control blocks the sandboxed renderer's `connect()` with ACCESS_DENIED before any DACL check — `connect()` on AF_UNIX opens the file for write. See `windows/win_acl.cpp` (`WinACL`). Refs: [AF_UNIX comes to Windows](https://devblogs.microsoft.com/commandline/af_unix-comes-to-windows/), [Mandatory Integrity Control](https://learn.microsoft.com/en-us/windows/win32/secauthz/mandatory-integrity-control). Only the connect side sets up a zmq monitor — wiring `zmq_socket_monitor` on the bind side breaks multi-gate on Windows (new bind never sees ACCEPTED).
...
| `network_broker` | inherited handle-restricted named pipe; socket handles via `WSADuplicateSocket` | inherited socketpair, child end at FD = `TG_BROKER_FD` | inherited socketpair, child end at FD = `TG_BROKER_FD` | All renderer socket creation + DNS resolution, mediated by the launcher's broker thread |
