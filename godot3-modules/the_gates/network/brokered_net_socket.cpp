// Ported from the 4.x fork's modules/the_gates/network/brokered_net_socket.cpp:
// POSIX only, against Godot 3's NetSocket interface.
...
Error BrokeredNetSocket::_ensure_connected(const IP_Address &p_ip, uint16_t p_port) {
...
Error BrokeredNetSocket::recvfrom(uint8_t *p_buffer, int p_len, int &r_read, IP_Address &r_ip, uint16_t &r_port, bool p_peek) {
	if (_sock == TG_INVALID_SOCK) {
		return FAILED;
	}
...
Error BrokeredNetSocket::sendto(const uint8_t *p_buffer, int p_len, int &r_sent, IP_Address p_ip, uint16_t p_port) {
...
Error BrokeredNetSocket::join_multicast_group(const IP_Address & /*p_multi_address*/, String /*p_if_name*/) {
	return ERR_UNAVAILABLE;
}

Error BrokeredNetSocket::leave_multicast_group(const IP_Address & /*p_multi_address*/, String /*p_if_name*/) {
	return ERR_UNAVAILABLE;
}
